...
{
    public class UpdateUserAvatarService : IRequestHandler<UpdateUserAvatarCommand, UserAvatarUpdatedEvent>
    {
        ...
        public async Task<UserAvatarUpdatedEvent> Handle(UpdateUserAvatarCommand command, CancellationToken cancellationToken)
        {
            if (command.file == null || command.file.Length == 0)
            {
                throw new ArgumentException("No file upload");
            }
            if (command.file.Length > 2 * 1024 * 1024)
            {
            ...
            try
            {
                using var imageStream = command.file.OpenReadStream();
            ...
            var fileName = $"{command.UserId}{fileExetension}";
